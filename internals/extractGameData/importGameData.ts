...
function metaClassFor(gameClass: string, group: string, shape: string): string {
  const key = normalize(gameClass.replace(/_(LD|HD|SO|SS|SW|RET)$/i, ''));
  if (META_ALIASES[key]) return META_ALIASES[key];
  if (/^f-?usa[_-]?gen1/i.test(gameClass)) return 'F-USA Gen.1';
  if (/^f-?usa[_-]?gen2/i.test(gameClass)) return 'F-USA Gen.2';
  if (/^f-?usa[_-]?gen3/i.test(gameClass)) return 'F-USA Gen.3';
  if (/^f-?usa[_-]?2022|^f-?usa[_-]?2023/i.test(gameClass)) return 'F-USA Gen.3';
  if (/^f-?classic[_-]?gen1/i.test(gameClass)) return 'Formula Classic G1';
  if (/^f-?classic[_-]?gen2/i.test(gameClass)) return 'Formula Classic G2';
  if (/^f-?classic[_-]?gen3|^f-?classic[_-]?gen4/i.test(gameClass)) return 'Formula Classic G3';
  if (/^f-?hitech[_-]?gen1/i.test(gameClass)) return 'Formula Classic G1';
  if (/^f-?hitech[_-]?gen2/i.test(gameClass)) return 'Formula Classic G2';
  if (/^f-?retro[_-]?gen1/i.test(gameClass)) return 'Formula Retro G1';
  if (/^f-?retro[_-]?gen2/i.test(gameClass)) return 'Formula Retro G2';
  if (/^f-?retro[_-]?gen3/i.test(gameClass)) return 'Formula Retro G3';
  if (/^f-?vintage[_-]?gen1/i.test(gameClass)) return 'Formula Vintage G1';
  if (/^f-?vintage[_-]?gen2/i.test(gameClass)) return 'Formula Vintage G2';
  if (/^f-?ultimate[_-]?gen1/i.test(gameClass)) return 'Formula Ultimate Gen1';
  if (/^f-?ultimate[_-]?gen2/i.test(gameClass)) return 'Formula Ultimate Gen2';
  if (/^f-?v10[_-]?gen1/i.test(gameClass)) return 'Formula V10 G1';
  if (/^f-?v10[_-]?gen2|^f-?v10[_-]?gen3/i.test(gameClass)) return 'Formula V10 G2';
  if (/^f-?v12/i.test(gameClass)) return 'Formula V12';
  if (/^f-?v8/i.test(gameClass) || /^f-?reiza/i.test(gameClass)) return 'Formula Reiza';
  if (/^f-?vee/i.test(gameClass)) return 'FormulaVee Brazil';
  if (/^f-?3|^f301|^f309/i.test(gameClass)) return 'F3';
  if (/^f-?trainer|^f-?junior|^f-?inter/i.test(gameClass)) return 'FTS';
  if (/^f5$/i.test(gameClass)) return 'FTS';
  if (/sprint|^rx$|^sst$|^stt$|dirt/i.test(gameClass)) return 'Sprint Race';
  if (/safetycar/i.test(gameClass)) return 'Street Cars A';
  if (/supercars|hypercars/i.test(gameClass)) return 'Street Cars B';
  if (/^tc60|^tc70|^st96/i.test(gameClass)) return 'GT Classics';
  if (/carrer[a]? ?cup/i.test(gameClass)) return 'Carrera Cup';
  if (/^gt2|^gtc|^trofeo|^super ?trofeo|^les_/i.test(gameClass)) return 'GT Classics';
  if (/fusa.*gen1/i.test(gameClass)) return 'F-USA Gen.1';
  if (/fusa.*gen2/i.test(gameClass)) return 'F-USA Gen.2';
  if (/fusa.*gen3/i.test(gameClass)) return 'F-USA Gen.3';
  if (/gt3/i.test(gameClass)) return 'GT3';
  if (/gt4/i.test(gameClass)) return 'GT4';
  if (/gt5/i.test(gameClass)) return 'GT5';
  if (/gte/i.test(gameClass)) return 'GTE';
  if (/gt1|gtr/i.test(gameClass)) return 'GT1';
  if (/lmp2|p2/i.test(gameClass)) return 'P2';
  if (/lmp3|p3/i.test(gameClass)) return 'P3';
  if (/lmp1|p1|lmdh|dpi/i.test(gameClass)) return 'P1';
  if (/kart/i.test(gameClass) || shape === 'Kart') return 'Karting 125CC';
  if (/touring|stock|copa|superv8|group a/i.test(`${gameClass} ${group}`)) return 'Group A';
  if (/historic openwheel/i.test(group)) return 'Formula Retro G2';
  if (/openwheel/i.test(shape) || /openwheel/i.test(group)) return 'Formula V10 G2';
  if (/road|street/i.test(`${shape} ${gameClass}`)) return 'Street Cars A';
  if (/prototype|lm/i.test(`${shape} ${group}`)) return 'P1';
  return 'GT Classics';
}
...
function main() {
  ...
  const rawCars = readCsv(path.join(SOURCE_DIR, 'bootflow_vehicle_details.csv'));
  const rawTracks = readCsv(path.join(SOURCE_DIR, 'bootflow_track_details.csv'));
  ...
  for (const row of rawCars) {
    ...
    seenCars.add(id);
    const metaClass = metaClassFor(row['Vehicle Class'], row['Vehicle Group'], row['Vehicle Shape']);
  ...
  const classes = [...new Map(cars.map(row => [row['Vehicle Class'], row])).values()].map(row => {
    const metaClass = row.meta_class;
    const classRow = classByName.get(metaClass);
    return {
      ...
      mapping_source: META_ALIASES[normalize(row['Vehicle Class'])] ? 'explicit-alias' : 'heuristic-family',
    };
  });
  ...
  for (const track of tracks) {
    const matchingApp = appTracks.find(app =>
    ...
    const mappedClasses = matchingApp
      ? Object.entries(matchingApp).filter(([, value]) => value === 'x').map(([name]) => name)
      : classRows.filter(row => {
          const type = track['Track Type'];
          if (type === 'Kart') return /kart/i.test(row.class);
          if (type === 'Rallycross') return ['Sprint Race', 'FTS', 'GT5'].includes(row.class);
          if (type === 'Oval') return /stock|copa|f-usa|super v8|montana/i.test(row.class);
          return true;
        }).map(row => row.class);
  ...
  for (const old of appTracks) {
    const match = tracks.find(row => normalize(row.TrackName) === normalize(old.name));
    legacyAliases.push({
      kind: 'track',
  ...
  console.log(`Imported ${cars.length} cars, ${tracks.length} tracks, ${classes.length} game classes`);
  console.log(`Generated ${trackMappings.length} track/class mappings and ${legacyAliases.length} legacy aliases`);
}
